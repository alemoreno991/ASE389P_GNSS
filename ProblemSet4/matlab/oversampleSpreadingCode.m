function [codeOS] = oversampleSpreadingCode(code,delChip,delOffset,Ns,Np)
...
%
% delOffset ------- The offset of the first sample from the code start time,
%                   in chips.  A positive (negative) value indicates that the
%                   first sample occurs after (before) the code start time.
%                   For acquisition, set this value to zero.
% 
...
codeOS = zeros(Ns,1);
for ii=1:Ns
  k = floor((ii-1)*delChip + delOffset);
  k = mod(k, Np);
  codeOS(ii) = code(k+1);
end